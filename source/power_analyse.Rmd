...
n_ind_per_loc <- identifications |>
  group_by(species_nm) |>
  summarise(
    mean = mean(no_ind, na.rm = TRUE),
    sd = sd(no_ind, na.rm = )
  )
```

```{r species per location}
n_spec_per_loc <- as.integer(pmax(
  0,
  rnorm(
    n_locations,
    avg_species_per_loc,
...
simulate_species_presence <- function(n_species,
  ...
  species <- tibble::tibble(
    ...
    presence_prob = ifelse(.data$species_id <= round(0.1 * n_species),
      general_prob,
      rare_prob
    ) # De eerste 20% zijn algemene soorten
  )
...
simulated_data <- expand.grid(
  ...
  mutate(
    ...
    observed_counts = list(simulate_population(
      year,
      location,
      n_species,
      decline_rate,
