...
test_data <- n_ind_per_loc |>
  ...
  mutate(month = substr(sample_code, 20, 20),
         location = substr(sample_code, 1, 8))         
```

```{r}
model1 <- glmer(n_ind ~ month + (1|location), family="poisson", data=test_data)
summary(model1)
...
generate_data <- function(n_locations) {
  ...
  months <- as.factor(5:9)
  extra_months <- as.factor(5:8)  # extra visits only allowed in months 5–8
  ...
  extra_visits <- do.call(rbind, lapply(locs, function(loc) {
    do.call(rbind, lapply(years, function(y) {
      data.frame(
        location = loc,
        year = y,
        month = sample(extra_months, size = 3, replace = FALSE),  # 3 distinct months
        stringsAsFactors = FALSE
      )
    }))
  }))
  ...
  full_data$month <- factor(full_data$month, levels = levels(months))  # ensure consistency
  return(full_data)
}
```
...
run_power_analysis <- function(year_slope_percent, n_locations, nsim = 100) {
  ...
  test <- powerSim(model, nsim = nsim, test = fixed("year", "z"), progress = FALSE)
...
power_df <- bind_rows(
  lapply(names(results), function(name) {
   summary(results[[name]]) |> 
      mutate(trend = str_extract(name, "([0-9]|\\.)+(?=_)"),
...
ggplot(power_df, aes(x = points, y = mean, color = trend)) +
  ...
  labs(title = "Power to Detect Yearly Trends",
