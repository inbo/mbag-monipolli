...
# enkel nodig indien je interactief werkt en de code vand deze chunks nodig hebt
...
ap_data_n_ind <- apoidea %>%
  ...
  group_by(
    location_code, method_cd, method_combi,
    spring_code, level, uv, maand, sample_code) %>%
  summarise(
    n_ind = sum(no_ind),
    .groups = "drop") %>%
  filter(n_ind < 500)
...
```{r}
ap_data_n_ind_spring <- ap_data_n_ind %>%
  filter(spring_code == "s.s.")
```


```{r}
ggplot(ap_data_n_ind_spring, aes(x = n_ind)) +
  geom_histogram()
```
...
marginaleffects::plot_predictions(
  ...
  labs(y = "Aantal individuen (met honingbijen)")
```
...
ap_data_n_ind <- apoidea %>%
  ...
  group_by(
    location_code, method_cd, method_combi, spring_code, uv,
    level, maand, sample_code) %>%
  summarise(n_ind = sum(no_ind), .groups = "drop")
```
...
marginaleffects::plot_predictions(
  ...
  labs(y = "Aantal individuen (zonder honingbijen)")
```
...
```{r}
ap_data_sp_rich_n_ind_spring <- ap_data_sp_rich_n_ind %>%
  filter(spring_code == "s.s.")
```


```{r}
ggplot(ap_data_sp_rich_n_ind_spring, aes(x = n_species)) +
  geom_histogram()
ggplot(ap_data_sp_rich_n_ind_spring,
       aes(x = n_ind)) +
  geom_point(aes(y = n_species)) +
  facet_wrap(~method_cd, scales = "free")
```
...
model0 <- glmmTMB(
  ...
  ziformula = ~1,
  na.action = na.fail,
  data = ap_data_sp_rich_n_ind_spring
)
```
