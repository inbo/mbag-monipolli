...
identifications <- identifications %>% 
  janitor::clean_names() %>% 
  rename(no_ind = "no_males_females",
...
```{r}
identifications %>% 
  group_by(order, familiy, species_nm) %>% 
  summarise(total_observations = sum(no_ind)) %>% 
  arrange(species_nm) %>% 
  DT::datatable()
```
...
apidae <- identifications %>% 
  as_tibble() %>%
  filter(familiy %in% c("Apidae", "Andrenidae",
...
exp_shannon <- apidae %>% 
  ...
  pivot_wider(names_from = species_nm,
                values_from = n,
                values_fn = sum,
                values_fill = 0) %>% 
  ungroup() %>% 
...
ap_data_sp_rich_n_ind <- apidae %>%
  ...
  filter(time_series == 0) %>%
  mutate(maand = as.factor(month(date_b))) %>% 
  select(location_code, method_cd, spring_code,
...
cont_ap <- xtabs(no_ind ~ sample_id + familiy, apidae)
...
polli <- identifications %>%
  filter(familiy %in% c("Apidae", "Andrenidae",
                        ...
                        "Melittidae", "Megachilidae",
                        "Syrphidae")) %>%
  mutate(group = ifelse(familiy == "Syrphidae", "Syrphidae", "Apoidae")) %>% 
  full_join(expanded_samples, by = c("sample_id", "sample_code", "group")) %>% 
  mutate(no_ind = ifelse(is.na(no_ind), 0, no_ind),
         maand = as.factor(month(date_b)),
         uv = ifelse(spring_code == "MP", "non-uv", "uv")) %>% 
  mutate(numbered_value = as.numeric(str_sub(sampling_site_cd,-2,-1))) %>%
  filter(numbered_value < 11,
         time_series == 0) %>% 
  group_by(location_code, method_cd, spring_code, level, maand, 
           group, uv, sample_id) %>% 
  summarise(n_species = n_distinct(species_nm, na.rm = TRUE),
            n_ind = sum(no_ind))
...
```{r}
model2 <- glmmTMB(n_species ~ spring_code * group + location_code + 
                    maand + n_ind,
                  family = "poisson",
                  ziformula = ~1,
                  na.action = na.exclude,
...
```{r}
model3 <- glmmTMB(n_species ~ uv * group + level * group + 
...
```{r}
model4 <- glmmTMB(n_species ~ c_meth * group +
                    location_code + maand + n_ind,
                  family = "poisson",
                  ziformula = ~1,
                  na.action = na.exclude,
