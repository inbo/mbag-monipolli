...
apoidea_richness %>%
  ...
  geom_boxplot(
    aes(y = n_species,
        x = spring_code,
        fill = factor(time_series))
  ) +
  facet_wrap(~method_cd, scales = "free_x")
```
...
rarefaction_result <- apoidea %>%
  ...
  group_by(location_code, method_cd, spring_code, level, date_b, species_nm) %>%
  summarise(n = sum(no_ind)) %>%
  filter(!is.na(species_nm)) %>%
  group_by(location_code, method_cd, spring_code, level) %>%
  nest() %>%
...
apoidea_cumulative <- apoidea %>%
  ...
  arrange(time_order, .by_group = TRUE) %>%
  group_by(
    location_code, sampling_site_cd, method_cd, spring_code, time_order
  ) %>%
  summarise(
...
apoidea_cumulative %>%
  ...
  facet_grid(location_code ~ spring_code)
```
...
rarefaction_result_2 <- apoidea %>%
  ...
  group_by(location_code, method_cd, spring_code, level, month,
...
rarefaction_result_2 %>%
  ...
  geom_ribbon(
    aes(
      ymin = lwr, ymax = upr, fill = spring_code), alpha = 0.2) +
  xlab("Rarefied number of sampling sites") +
  ylab("Number of (morpho)species") +
...
cont_ap <- xtabs(no_ind ~ sample_id + family, apoidea)
