...
```{r pwc-method zonder honingbij}
emmeans::emmeans(model2, pairwise ~ method_combi)
```

```{r pwc-method-taxgroup zonder honingbij}
emmeans::emmeans(model2, pairwise ~ method_combi * taxgroup)
```

```{r pwc-location zonder honingbij}
emmeans::emmeans(model2, pairwise ~ location_code)
```

```{r pwc-location-taxgroup zonder honingbij}
emmeans::emmeans(model2, pairwise ~ location_code * taxgroup)
```
...
```{r, fig.cap = "Verwacht aantal soorten (zonder honingbijen) in een pantrap-set of transectsegment van 50 m per soortgroep en methode op basis van ons model."}
marginaleffects::plot_predictions(
  model2,
  condition = c("method_combi", "taxgroup"),
  type = "response",
...
```{r}
apoidea_cumulative_geen_honingbij <- apoidea %>%
...
apoidea_cumulative_geen_honingbij <- apoidea_cumulative_geen_honingbij |>
  mutate(
...
```{r}
model5 <- glmmTMB(prop ~ time_order * method_combi + (1|sampling_site_method),
                  family = binomial,
                  weights = n_species_final,
                  data = apoidea_cumulative_geen_honingbij)
```
...
```{r}
model6 <- glmmTMB(prop ~ time_order * method_combi +
                    #location_code +
