...
purrr::pmap(
  ...
  function(title = title, group = group, familyvec = familyvec) {
    knit_expand(
      "_verkenning.Rmd",
      title = title,
      group = group,
...
em <- emmeans::emmeans(model1, pairwise ~ (method_combi | taxgroup))
em$contrasts
```
...
```{r, fig.cap = "Verwacht aantal soorten wilde bijen in een pantrap-set of transectsegment van 50 m per soortgroep en methode op basis van ons model."}
pred_spec <- marginaleffects::plot_predictions(
  model1,
...
pred_spec |>
  ...
  scale_x_discrete(
    ...
    labels = c(
      "Pan trap: Non-UV, soil surface",
      "Pan trap: UV, soil surface",
      "Pan trap: UV, vegetation",
      "Transect: visual detection",
      "Transect: vegetation sweeping"
    )
  ) +
  geom_signif(
    comparisons = list(
    ...
    y_position = c(1, 1.05, 1, 1.1, 1.15, 1.2),
    tip_length = 0.01,
    vjust = 0.5
  )
```

```{r, fig.cap = "Verwacht aantal soorten zweefvliegen in een pantrap-set of transectsegment van 50 m per soortgroep en methode op basis van ons model."}
pred_spec |>
  filter(taxgroup == "Syrphidae") |>
  ggplot(aes(
    x = method_combi,
    y = estimate,
    group = method_combi
  ...
  geom_signif(
    ...
    y_position = c(0.3, 0.325, 0.35, 0.375),
    tip_length = 0.01,
    vjust = 0.5
  )
```
...
```{r pwc-month-taxgroup}
em <- emmeans::emmeans(model1, pairwise ~ (maand | taxgroup))
em$contrasts
```

```{r, fig.cap = "Verwacht aantal soorten wilde bijen in een pantrap-set of transectsegment van 50 m per maand en soortgroep op basis van ons model."}
p <- marginaleffects::plot_predictions(
  model1,
  condition = c("maand", "taxgroup"),
  type = "response",
  vcov = TRUE,
  draw = FALSE
)
...
p |>
  ...
  geom_signif(
    ...
    y_position = c(2.1, 2.25, 2, 2.2, 2.05, 2.15, 2),
    tip_length = 0.01,
    vjust = 0.65
  )
```

```{r, fig.cap = "Verwacht aantal soorten zweefvliegen in een pantrap-set of transectsegment van 50 m per maand en soortgroep op basis van ons model."}
p |>
  filter(taxgroup == "Syrphidae") |>
  ggplot(aes(x = maand, y = estimate)) +
  geom_pointrange(
  ...
  geom_signif(
    ...
    y_position = c(1.15, 1, 1.2, 1.05, 1.25, 1.1),
    tip_length = 0.01,
    vjust = 0.65
  )
```
...
```{r, fig.cap = "Verwacht aantal individuen wilde bijen (zonder honingbijen) in een pantrap-set of transectsegment van 50 m per soortgroep en methode op basis van ons model."}
...
pred_ind |>
  ...
  geom_signif(
    ...
    y_position = c(1.35, 1.4, 1.45, 1.35),
    tip_length = 0.01,
    vjust = 0.5
  )
```

```{r, fig.cap = "Verwacht aantal individuen zweefvliegen in een pantrap-set of transectsegment van 50 m per soortgroep en methode op basis van ons model."}
pred_ind |>
  filter(taxgroup == "Syrphidae") |>
  ggplot(aes(
    x = method_combi,
  ...
  geom_signif(
    ...
    y_position = c(0.46, 0.50, 0.52, 0.54, 0.48),
    tip_length = 0.01,
    vjust = 0.5
  )
```
...
emmeans::emmeans(model2, pairwise ~ (method_combi | taxgroup))
```
...
```{r pwc-month-taxgroup-n-ind}
em <- emmeans::emmeans(model2, pairwise ~ (maand | taxgroup))
em$contrasts
```

```{r, fig.cap = "Verwacht aantal individuen wilde bijen in een pantrap-set of transectsegment van 50 m per maand en soortgroep op basis van ons model."}
p <- marginaleffects::plot_predictions(
  model2,
  condition = c("maand", "taxgroup"),
  type = "response",
...
p |>
  ...
  geom_signif(
    ...
    y_position = c(3.4, 3.7, 3.2, 3.6, 3.3, 3.5, 3.4),
    tip_length = 0.01,
    vjust = 0.65
  )
```

```{r, fig.cap = "Verwacht aantal individuen zweefvliegen in een pantrap-set of transectsegment van 50 m per maand en soortgroep op basis van ons model."}
p |>
  filter(taxgroup == "Syrphidae") |>
  ggplot(aes(x = maand, y = estimate)) +
  geom_pointrange(
  ...
  geom_signif(
    ...
    y_position = c(1.6, 1.7, 1.85, 1.65, 1.75, 1.6, 1.8, 1.65),
    tip_length = 0.01,
    vjust = 0.65
  )
```
...
flowers <- flowers |>
  mutate(
    location_code = str_sub(sampling_site_cd, start = 1L, end = 8L),
...
emmeans::emmeans(
  model4,
  ~ log(number_of_floral_units + 1) * group,
  infer = TRUE
)
emmeans::emtrends(model4, ~group, var = "number_of_floral_units")
...
```{r}
model5 <- glmmTMB(
  prop ~ log(cumulative_days) * method_combi +
...
Verwacht percentage van de "totale" gemeenschap dat gevonden wordt voor het aantal dagen dat pan traps geplaatst worden:
```{r}
emm <- emmeans::emmeans(
  model5,
  pairwise ~ cumulative_days,
  type = "response",
  at = list(cumulative_days = c(
    0.25,
    1.25,
    2.25,
    3.25,
...
confint(emm$contrasts, adjust = "sidak")
```

```{r time-series-wild-bees, fig.cap = "Proportie soorten gevonden over tijd voor wilde bijen."}
...
```{r}
model6 <- glmmTMB(
  prop ~ cumulative_days * method_combi +
    (1 | sampling_site_method),
  family = binomial,
  weights = n_species_final,
...
```{r time-series-hoverflies, fig.cap = "Proportie soorten gevonden over tijd voor zweefvliegen."}
...
model8 <- glmmTMB(
  cumulative_n_species ~
    log(cumulative_days) * method_combi +
...
```{r}
performance::check_predictions(model9)
performance::check_overdispersion(model9)
performance::check_collinearity(model9)
performance::check_residuals(model9)
```
...
# berekening slopes voor elk method_combi level
slopes <- emmeans::emtrends(
  model9,
  specs = "method_combi",
  var = "log(cumulative_days)"
)

# summary voor de slopes
summary(slopes, infer = TRUE)
```

Test of de verschillen tussen slopes significant zijn
...
emmeans::emmeans(
  model9,
  consec ~ cumulative_days | method_combi,
  type = "response",
  at = list(
    cumulative_days = c(
      0.25,
...
time_data |>
  ...
  add_row(
    cost_data |>
      ...
      select(Activiteit = Kostencategorie, methode, submethode, prijs)
  ) |>
  arrange(submethode) |>
...
pt_ss <- time_data |>
  ...
  add_row(
    cost_data |>
      filter(submethode == "s.s.") |>
      select(Activiteit = Kostencategorie, methode, submethode, prijs)
  ) |>
  mutate(prijs_per_locatie = prijs)

pt_ss |>
  kableExtra::kable(digits = 0)
```

De prijs per locatie zou als volgt toenemen (verplaatsingskost niet meegerekend) wanneer we 1 tot 5 pan traps plaatsen gedurende 1 tot 7 maanden:
...
for (i in maanden) {
  for (j in pantraps) {
    x <- pt_ss[c(1, 4), ] |>
      mutate(berekening = prijs * j) |>
      summarise(m = sum(berekening)) |>
      pull(m)

    y <- pt_ss[c(2, 3), ] |>
      mutate(berekening = prijs * i * j) |>
...
Als we twee pan traps s.s.(uv - vegetatie) en twee pan traps SSL (uv - bodem) zouden plaatsen zouden de prijs per locatie als volgt zijn voor 1 tot 7 verschillende maanden monitoring:
...
Als we één transect s.s. (geen sweeping) van 250 m per locatie zouden lopen zouden we volgende kosten hebben per locatie:

```{r}
ts_ss <- time_data |>
  filter(methode == "TS", submethode == "s.s.") |>
  mutate(prijs_per_locatie = prijs * 5)
...
Per locatie: twee pan traps s.s. (uv - vegetatie) en twee pantraps SSL (uv - bodem) die drie maal per jaar een week in het veld worden gelaten. Daarnaast wordt ook elke keer een transect van 250 m gelopen.

1: dit gebeurt door veldwerkers met manuele identificatie

-   Verplaatsingskosten:

-   Kilometervergoeding: `r round(verpl * 6)` euro
-   Loonkost: `r round(verpl_tijd * loon_verpl * 6)` euro

-   Aankopen, plaatsen, inzamelen en uitsorteren van 2 pan traps s.s. en 2 pan traps SSL op drie momenten: 545 euro per locatie

-   Transect lopen en uitsorteren op drie momenten: 115 euro per locatie

-   Identificeren van soorten: `r round(iden_cost * 9 * 3)` euro

-   Totale kost: 1877 euro per jaar per locatie
...
-   Aankopen pan traps: 12 euro per locatie

-   Transect lopen: 0 euro

-   Identificeren van soorten: 70 euro per locatie

-   Totale kost: 82 euro per jaar per locatie
